...
import { ArrowLeft, CheckCircle2, Circle, Plus, Archive, Edit, Save, X } from "lucide-react";
...
const handleToggleSubtask = async (subtask: Subtask) => {
    ...
    updateProjectInStore({ id: project.id, subtasks: updatedSubtasks });
        ...
        <div className="p-6 border-b border-skin-border">
            ...
            <div className="flex justify-between items-start">
                ...
                <div className="flex-shrink-0 ml-4 flex items-center gap-2">
                     {isEditing ? (
                        <>
                            <button onClick={handleEditSave} className="p-2 text-slate-500 hover:text-emerald-600" title="Save Changes"><Save size={20}/></button>
                            <button onClick={() => setIsEditing(false)} className="p-2 text-slate-500 hover:text-red-600" title="Cancel"><X size={20}/></button>
                        </>
                     ) : (
                        <>
                            <button onClick={() => setIsEditing(true)} className="p-2 text-slate-500 hover:text-sky-600" title="Edit Project"><Edit size={20}/></button>
                            <button onClick={handleArchiveProject} className="p-2 text-slate-500 hover:text-red-600" title="Archive Project"><Archive size={20} /></button>
                        </>
                     )}
                </div>
            </div>
